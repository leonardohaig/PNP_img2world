...
int main029()
{
	...
	cv::Mat imgM;
	cv::Mat groundM;
	fs["imgPointMatrix"] >> imgM;
	fs["groundPointMatrix"] >> groundM;
	...
	solvePnP(groundM, imgM, camera_matrix, distortion_coefficients, rvec, tvec, false, CV_P3P);			//Gao的方法可以使用任意四个特征点，特征点数量不能少于4也不能多于4
	...
	cv::FileStorage fs1;
	fs1.open("../RTMatrix.xml", cv::FileStorage::WRITE);
	fs1 << "RMatrix" << rotM;
	fs1 << "TMatrix" << tvec;
	fs1.release();
	...
	Points3D.push_back(cv::Point3f(-1650, 17000, -1800));		//P1 三维坐标的单位是毫米
